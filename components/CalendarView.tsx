...
export function CalendarView() {
  const { selectedState, setSelectedState, currentYear, currentMonth, setCurrentYear, setCurrentMonth } = useCalendar();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => { setCurrentMonth(today.getMonth()); setCurrentYear(today.getFullYear()); }}>
            Heute
          </Button>
        </div>
      ...
      <div className="rounded-lg border overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          {days.map((day, i) => {
            if (!day) return <div key={i} className="h-16 border-t border-r last:border-r-0 bg-muted/30" />;
      ...
      <div className="space-y-1">
        <h3 className="font-medium text-sm">Feiertage im {MONTH_NAMES[currentMonth]}</h3>
        {holidays
          .filter((h) => h.date.startsWith(`${currentYear}-${String(currentMonth + 1).padStart(2, "0")}`))
          .map((h) => (
            <div key={h.date} className="flex items-center gap-2 text-sm">
              <span className="text-muted-foreground w-24">{new Date(h.date).toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit" })}</span>
              <span>{h.name}</span>
            </div>
          ))}
      </div>
